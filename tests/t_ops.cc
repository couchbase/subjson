...
TEST_F(OpTests, testOperations)
{
    ...
    ASSERT_EQ(SUBDOC_SUCCESS, performNewOp(op, SUBDOC_CMD_GET, "name"));
    ASSERT_EQ("\"Allagash Brewing\"", t_subdoc::getMatchString(op->match));
    ASSERT_EQ(SUBDOC_SUCCESS, performNewOp(op, SUBDOC_CMD_EXISTS, "name"));

    uint16_t rv = performNewOp(op, SUBDOC_CMD_DELETE, "address");
    ASSERT_EQ(SUBDOC_SUCCESS, rv);

    getAssignNewDoc(op, newdoc);
    // Should return in KEY_ENOENT
    ASSERT_EQ(SUBDOC_PATH_ENOENT, performNewOp(op, SUBDOC_CMD_GET, "address"));
    ...
    getAssignNewDoc(op, newdoc);
    ASSERT_EQ(SUBDOC_SUCCESS, performNewOp(op, SUBDOC_CMD_GET, "address"));
    ...
    ASSERT_EQ(SUBDOC_PATH_ENOENT, rv);
...
TEST_F(OpTests, testUnique)
{
    ...
    ASSERT_EQ(SUBDOC_DOC_EEXISTS, rv);
    ...
    ASSERT_EQ(SUBDOC_PATH_MISMATCH, rv);
...
TEST_F(OpTests, testNumeric)
{
    ...
    ASSERT_EQ(SUBDOC_DELTA_E2BIG, rv);
...
TEST_F(OpTests, testValueValidation)
{
    ...
    ASSERT_EQ(SUBDOC_VALUE_CANTINSERT, rv);
