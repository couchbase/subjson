...
static std::string getComponentString(const Path& nj, int ix) {
    const auto& comp = nj.get_component(ix);
    return {comp.pstr, comp.len};
}

static unsigned long getComponentNumber(const Path& nj, int ix) {
...
TEST_F(PathTests, testEscapes) {
    Path ss;
    std::string pth = "`simple`.`escaped`.`path`";
    ASSERT_EQ(0, ss.parse(pth));
...
TEST_F(PathTests, testGetRootType) {
    ...
    ASSERT_EQ(JSONSL_T_OBJECT,
              Util::get_root_type(Command::ARRAY_PREPEND, "a"));
    ...
    ASSERT_EQ(JSONSL_T_LIST,
              Util::get_root_type(Command::ARRAY_PREPEND, "[-1]"));
}
